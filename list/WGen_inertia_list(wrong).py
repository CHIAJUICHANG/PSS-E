...
fout=open(r"C:\Program Files\114\txtfile\WGen_inertia_list.txt",'w')
savFn, snpFn = psspy.sfiles()
fout.write('MODEL: Load flow:     %s\n       Dynamic model: %s\n\n' % (savFn,snpFn))
fout.write('   BUS    NAME              GEN   MODEL   H(sec)   MBASE      PGEN     QGEN\n-----------------------------------------------------------------------------\n')
...
for iM in range(0,Nmach):
    ...
    if WgenMdl == None :continue #not WGen, is Gen
    else :
        ierr, icon0 = psspy.windmind(ibus, genId, 'WMECH', 'CON')
        ierr, WmechMdl = psspy.windmnam(ibus, genId, 'WMECH')
        if WmechMdl == None :  #WGen, no Mechanical
           WmechMdl="None"
           H=0
        else :
            WmechMdl=WmechMdl.strip()
            if WmechMdl=='WT12T1': iH=icon0                           
            if WmechMdl=='WT3T1': iH=icon0+1    
            ierr,H=psspy.dsrval('CON', iH)
    fout.write ('%6d  %12s   %2s  %6s  %6.3f  %8.3f  %7.3f  %7.3f\n' % (ibus,busN,genId,WmechMdl,H,machloadss,machloadpp,machloadqq))
fout.close()
